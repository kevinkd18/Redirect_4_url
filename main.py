...
from pymongo.server_api import ServerApi, MongoClient
...
def save_subscription(chat_id, unique_id):
    """Save the user's subscription with a 1-minute expiration in IST."""
    ist = pytz.timezone('Asia/Kolkata')
    subscribed_until = datetime.now(ist) + timedelta(minutes=1)
...
def send_subscription_message(chat_id, unique_id):
    ...
    try:
        ...
        greeting_text = (
            ...
            "*What is the token?*\n"
            "This is an ads token. After completing the process, you can use the bot for 10 minutes."
        )
...
def verify_success(unique_id):
    ...
    try:
        ...
        subscribed_until = datetime.utcnow() + timedelta(minutes=1)
        users_collection.update_one(
            {"unique_id": unique_id},
        ...
        bot.send_message(
            chat_id,
            "🎉 *Subscription successful!* You can now use the bot for the next 10 minutes. 😊",
            parse_mode="Markdown"
        )
...
# --- Main ---
def set_webhook():
    """Forcefully set the webhook for the bot."""
    webhook_url = f"{WEBHOOK_URL.rstrip('/')}/{BOT_TOKEN}"
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/setWebhook?url={webhook_url}"
    try:
        response = requests.get(url)
        response_json = response.json()
        print(f"Set Webhook Response: {response_json}")
        if response_json.get("ok"):
            print("Webhook set successfully.")
        elif response_json.get("error_code") == 429:
            print("Too many requests. Webhook already set.")
        else:
